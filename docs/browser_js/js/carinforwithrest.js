function carTableBody(data) {
  ...
  for (let car_hashmap of carinfor_obj) {
    ...
    console.log(`YEAR: ${car_hashmap.YEAR}, CAR_NAME: ${car_hashmap.CAR_NAME}, 
                  CAR_INFOR_ID: ${car_hashmap.CAR_INFOR_ID}, COMPANY_ID: ${car_hashmap.COMPANY_ID}`);
    // 동적으로 생성된 HTML 테이블 행을 추가
    outHtml = `${outHtml}<tr><td>${car_hashmap.YEAR}</td><td>${car_hashmap.CAR_NAME}</td>
                              <td>${car_hashmap.CAR_INFOR_ID}</td><td>${car_hashmap.COMPANY_ID}</td></tr>`;
  }
...
keydownObject.addEventListener('keydown', (event) => {
  if (event.code == 'Enter') {
    ...
    // carTableBody();
    getSelectAllFromREST();   // input box 누르면 실행
  }
});
